...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml" xmlns:umi="http://www.umi-cms.ru/TR/umi">
	...
	<xsl:template name="makeThumbnailSqueeze">
		...
		<xsl:variable name="thumbnail" select="document(concat('udata://system/makeThumbnailFull/(',$path,')/',$width,'/',$height,'/void/0/1/5/0/100'))/udata"/>
		<xsl:choose>
			<xsl:when test="not($thumbnail/src) and ($no-empty-image = 1)">
				<span class="hidden">
		...
		<img>
			...
			<xsl:attribute name="src">
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:value-of select="document(concat('udata://system/makeThumbnailFull/(.',$empty-image,')/',$empty_width,'/',$empty_height,'/void/0/0/'))/udata/src" />
					</xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>
